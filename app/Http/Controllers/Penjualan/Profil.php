...
class Profil extends Controller
{
    public function index(){
        ...
        return view('penjualan.profil.index',compact('data'));
    }
    public function prosesEditData(Request $req){
      $validator = Validator::make($req->all(),[
        ...
        if (!$dataLama) {
          return redirect()->to(url("Penjualan/profil"))->withErrors(["system"=>"User tidak Ditemukan"]);
        }
        ...
        if($simpan){
          return redirect()->to(url('Penjualan/profil'))->with('success', 'User berhasil diedit');
        }
    ...
    public function prosesEditPassword(Request $req){
        $validator = Validator::make($req->all(),[
          ...
          $simpan=$User->save();
          if($simpan){
            return redirect()->to(url('Penjualan/profil'))->with('success', 'Password berhasil diedit');
          }
